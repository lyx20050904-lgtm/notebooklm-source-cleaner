...
- 用户文档：`README.md`（仅维护用户可感知变更）
- 研发文档：`README.dev.md`（维护技术实现、压测与排障细节）

## P0 稳定性产品化落地状态（2026-04-13）

本轮已按 luwei 的 P0 规划完成以下落地：

1. 事件链路防回归基建
- 将 checkbox 事件防护抽象为事件拦截模块（`setupCheckboxInterception`）。
- 统一拦截 `pointerdown/mousedown/mouseup/click` 捕获阶段，避免误跳转回归。

2. 失败项结构化上报（本地）
- 新增失败分类：`EVENT_CONFLICT`、`SELECTOR_MISS`、`DIALOG_TIMEOUT`、`ROUTE_REMOUNT_FAIL`。
- 提供本地失败记录能力：`recordFailure`、`nlmGetFailures()`。

3. 事件链路调试开关与主题诊断
- 新增 `nlmSetDebug(true|false)` 控制调试输出。
- 在 `nlmDebug()` 中输出主题 token、事件拦截计数和近期失败摘要。

4. 回归验证
- 自动化压测结果：6/6 通过（含 `TC-P0-02-B` mousedown 链路回归）。
...
### 测试背景
针对用户反馈的两项严重问题执行压力测试：
1. 浅色模式下文本可读性异常（白字融入背景）
2. 批量模式下左侧圆点点击后误触发来源详情

本轮测试基于本地自动化压测脚本 `scripts/stress-test.js`（JSDOM 模拟 NotebookLM 结构）执行，重点覆盖：
- P0：浅色模式可读性、复选框可点击性与防跳转
- P1：快速开关压力、大规模来源注入压力
...
失败项（P0 阻塞）：
- `TC-P0-02-B` 复选框点击不触发来源跳转（mousedown 链路）
  - 期望：source 行 `mousedown` 导航计数 = 0
  - 实测：`downNavCount=1`
  - 结论：当前仅拦截 `click` 链路不足以覆盖宿主页在 `mousedown/pointerdown` 阶段触发的导航逻辑。

通过项摘要：
- `TC-P0-02-A` 复选框 click 可选中：通过
- `TC-P0-01-A` 浅色模式文字可读（模拟环境）：通过
- `TC-P0-01-B` 宿主样式覆盖压力（模拟环境）：通过
- `TC-P1-04` 快速切换 120 次：通过
- `S2-LARGE-500` 500 条来源注入：通过（约 1075ms）

### 复盘结论
- 当前版本存在 1 个 P0 阻塞缺陷：事件拦截覆盖不完整。
- 版本门禁结论：**No-Go**（P0 未清零，不允许发布）。

### 根因分析
- 交互防护只在 `click` 捕获阶段执行。
- 当宿主页将“进入来源详情”的逻辑绑定在 `mousedown` 或 `pointerdown` 时，复选框交互前即触发导航，导致用户感知为“圆点不可点”。

### 修复计划（复盘行动）
#### P0（必须先做）
1. 扩展全局捕获拦截到 `pointerdown`、`mousedown`、`mouseup`、`click` 四条链路。
2. 对 `.nlm-source-checkbox` 命中时统一执行：`stopPropagation()` + `stopImmediatePropagation()`（必要时）+ 条件 `preventDefault()`。
3. 增加回归用例：
   - 点击、双击、快速连击、边滚动边点击
   - 父级绑定 `mousedown` 与 `pointerdown` 的场景

#### P1（主链路稳定）
1. 在 `nlmDebug()` 中增加事件链路诊断输出（记录触发阶段和目标节点）。
2. 将 120 次快速开关、500 条注入作为发布前固定回归。

#### P2（长期治理）
1. 增加截图比对（light/dark）作为视觉回归。
2. 建立失败项分类统计（事件冲突/选择器失效/弹窗超时）。
...
### 执行说明（边界）
由于 NotebookLM 线上环境依赖真实账号与动态页面，本轮自动化采用本地模拟 DOM 压测验证“交互链路与注入性能”问题；线上环境仍需补充一次手工复核（light/dark + 实账号路由切换）作为发布前最终签收。

### 修复后复测（vNext）
已根据复盘计划完成事件链路修复并执行下一版自动化压测（`node scripts/stress-test.js`）：

- 结果：6/6 全通过（P0=0）
- 关键修复：
  - Checkbox 点击逻辑取消 `preventDefault`，保留默认勾选行为。
  - 新增 `pointerdown/mousedown/mouseup/click` 捕获阶段统一拦截。
  - 命中 checkbox 时执行 `stopPropagation + stopImmediatePropagation`，并在 down 阶段 `preventDefault` 阻断宿主页预导航动作。
- 回归结论：
  - `TC-P0-02-B` 从 `downNavCount=1` 降为 `downNavCount=0`。
  - 大规模注入（500 条）与快速开关（120 次）保持通过。

发布建议更新：
- 自动化门禁已满足上线条件。
- 仍需执行 1 轮线上实账号手工签收（浅色/深色 + 路由切换 + 连续勾选）后再发版。
