...
为了解决“浏览器暗色、NotebookLM 亮色，但插件仍显示暗色”的实际问题，当前代码已经做了两层修正：

1. 删除了“站内主题与系统主题不一致时，强制采用系统主题”的错误分支。
2. 在显式主题标记找不到时，新增“根据 NotebookLM 页面实际计算样式（背景色 / color-scheme）推断主题”的回退路径。

这意味着：

- 如果 NotebookLM 自己明确是亮色，插件会优先跟随亮色。
- 如果 NotebookLM 没有暴露明确主题标记，插件会根据页面自身样式判断。
- 只有在站内与页面样式都无法判断时，才回退到浏览器系统主题。

这次修复的重点不是“再加一个浅色覆盖”，而是“把错误的系统优先逻辑拆掉，改成 NotebookLM 优先”。